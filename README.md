...
| 6 | `#chk-testbead` | Test bead drop | Test | M3 S100 → dwell → M5, user confirms |
...
Step 6: Test bead drop
  ...
  → App sends:  M5\n               (feeder OFF)
  → GRBL replies: ok
  → Browser shows confirm dialog: "Did a bead drop?"
  → User clicks OK → chk-testbead ticks ✓
...
| `G0 X{x} Y{y} F{f}` | Rapid move to position | `ok` |
| `G4 P{ms}` | Dwell for N milliseconds | `ok` (after dwell) |
| `M3 S100` | Spindle CW at speed 100 → black feeder ON | `ok` |
| `M4 S100` | Spindle CCW at speed 100 → white feeder ON | `ok` |
| `M5` | Spindle stop → feeder OFF | `ok` |
...
; Row 0  (L→R)
M3 S100      ← black feeder ON (first bead is black)
G0 X3.50 Y3.50 F3000
G4 P300
M5           ← stop feeder before switching color
M4 S100      ← white feeder ON
G0 X10.50 Y3.50 F3000
G4 P300
...
...
## 11. Bead Feeder Control

The machine has two bead feeders controlled via the GRBL spindle output:

| Command | Spindle direction | Feeder | Bead color |
|---|---|---|---|
| `M3 S100` | CW (forward) | Feeder A | Black |
| `M4 S100` | CCW (reverse) | Feeder B | White |
| `M5` | Stop | Both off | — |

**Switching logic:** Feeder is only switched when the next bead's color differs from the current active feeder. `M5` is always sent before switching direction (never M3 → M4 directly). This produces the minimum number of feeder commands.

**Example sequence for adjacent beads of alternating colors:**
```
M3 S100       ← black ON
G0 X3.50 Y3.50 F3000
G4 P300
M5            ← stop before switch
M4 S100       ← white ON
G0 X10.50 Y3.50 F3000
G4 P300
M5            ← stop before switch
...
**Dwell (`G4 P{ms}`):** Sent after every `G0` move, regardless of feeder state. This gives the feeder time to release a bead into the tray. Default 300 ms, configurable 10–9999 ms.
...
| `M3 S{speed}` | Enable spindle CW / bead feeder A; respond `ok` |
| `M4 S{speed}` | Enable spindle CCW / bead feeder B; respond `ok` |
| `M5` | Stop spindle / feeders; respond `ok` |
...
### 16.3 Spindle → Feeder Mapping

The ESP32 firmware must map GRBL spindle commands to the bead feeder motors:

```
M3 S100  →  GPIO pin / PWM for Feeder A (black beads) — rotate motor CW
M4 S100  →  GPIO pin / PWM for Feeder B (white beads) — rotate motor CCW
M5       →  Both feeder motors stop
```

The speed value (`S100`) is sent but the feeders may operate at a fixed speed regardless of the S value, or use it as a PWM duty cycle — depending on hardware design.
...
| Bead feeders | 2× actuators (stepper/servo/solenoid) controlled via spindle outputs |
| - Feeder A | Black beads — M3 S100 (spindle CW) |
| - Feeder B | White beads — M4 S100 (spindle CCW) |
