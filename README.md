...
| `M3 S120` | Servo → Left bead | `ok` |
| `M3 S40` | Servo → Right bead | `ok` |
| `M3 S79` | Servo → Rest (neutral) | `ok` |
...
;   M3 S40   →  Right bead
;   M3 S79   →  Rest (neutral)
;   BLACK bead = left side (M3 S120)
;   WHITE bead = right side (M3 S40)

G21            ← mm units
G90            ← absolute positioning
M3 S79         ← servo to rest before homing
G28            ← home all axes (machine goes to 0,0,0)

; Row 0  (L→R)
G1 X3.50 Y3.50 F800    ← controlled move to slot
G4 P0                  ← full stop before drop
M3 S120                ← drop black bead (left side)
G4 P0.300              ← dwell 300 ms
M3 S79                 ← servo back to rest

G1 X10.50 Y3.50 F800
G4 P0
M3 S40                 ← drop white bead (right side)
G4 P0.300
M3 S79
...
...
| Command | Action |
|---|---|
| `M3 S120` | Servo → Left position |
| `M3 S40` | Servo → Right position |
| `M3 S79` | Servo → Rest (neutral) |

These values are the defaults and can be fine-tuned live in the **Servo Calibration** panel (machine panel, section 2.5) without reloading the page.

### Servo Calibration panel

The Servo Calibration section lets you dial in S-values interactively while connected:

1. Edit **S Left**, **S Rest**, **S Right** inputs.
2. Click **Test Black** or **Test White** — fires the servo (using the current side assignment) with the current dwell time, then returns to rest. No machine movement.
3. Adjust and repeat until the bead drops reliably.
4. Click **Apply to GCode** — updates the active S\_LEFT / S\_REST / S\_RIGHT used by `buildGcodeLines()` and marks the checklist bead-drop step as done.
...
|---|---|---|
| Black=Left, White=Right | `M3 S120` | `M3 S40` |
| Black=Right, White=Left | `M3 S40` | `M3 S120` |
...
**Startup:** `M3 S79` is sent before `G28` so the servo is always in the rest position before homing begins.
...
| - Left position | `M3 S120` |
| - Right position | `M3 S40` |
| - Rest (neutral) | `M3 S79` |
...
### Servo Calibration (machine panel)

| Setting | HTML ID | Default | Min | Max | Step |
|---|---|---|---|---|---|
| S Left | `#servo-s-left` | 120 | 0 | 1000 | 1 |
| S Rest | `#servo-s-rest` | 79 | 0 | 1000 | 1 |
| S Right | `#servo-s-right` | 40 | 0 | 1000 | 1 |
...
### v11 — Recalibrated servo values + interactive Servo Calibration panel

#### Machine / G-code
- **Recalibrated S values** — physically re-tested servo positions updated:
  - `M3 S120` → Left bead (was S110)
  - `M3 S40`  → Right bead (was S22)
  - `M3 S79`  → Rest / neutral (was S60)

#### UI — Servo Calibration section (machine panel)
- **New Servo Calibration panel** — dedicated section between Pre-run Checklist and Machine State
- **Live S-value inputs** — `S Left`, `S Rest`, `S Right` editable number inputs (default: 120 / 79 / 40)
- **Test Black / Test White buttons** — fires the servo at current input values (no machine movement) so operator can iterate until bead drops cleanly
- **Apply to GCode button** — commits tested values to `S_LEFT`, `S_REST`, `S_RIGHT` variables used by `buildGcodeLines()` and auto-checks the bead-drop checklist step
- **Applied confirmation** — inline status line shows the locked-in values after apply
